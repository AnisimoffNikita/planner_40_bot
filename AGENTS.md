...
- `block.multiple` — `true` для повторяемых блоков.
...
- Для `multiple: false` хранить объект `{fields: {...}}`.
- Для `multiple: true` хранить список экземпляров.
- Пустой `multiple`-блок не считается незаполненным и не должен попадать в PDF/status.
...
Пустые `multiple: true` блоки не показывать вообще. Например, если нет добавленных объявлений, блок `Объявления` не должен попадать в PDF и не должен подсвечиваться.
...
- `/set <block_id>.<field_id> <value>` — предложить изменение одиночного блока.
- `/add <block_id> <title>` — добавить экземпляр multiple-блока.
- `/set <block_id>[<entry_id>].<field_id> <value>` — предложить изменение экземпляра multiple-блока.
- `/delete <block_id>[<entry_id>]` — предложить удаление экземпляра multiple-блока.
...
- нужен ли entry_id для multiple-блока;
...
- PDF не должен включать пустые `multiple`-блоки.
...
2. `multiple: true` пустой блок не попадает в статус/PDF.
